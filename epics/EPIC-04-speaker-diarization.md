...
| 2026-05-09 | Claude Agent | **EPIC-04 fully closed (04a + 04b)**. Swift `DiarizationService` bridge shipped: Foundation Process bridge with FileHandle.bytes.lines streaming, AsyncTaskQueue serialization (with cancellation propagation fix that pre-existed since EPIC-03), withTaskCancellationHandler-based SIGTERM, JSON envelope decode with schema-version enforcement. Production entitlements gained the +5 EPIC-04b additions; child binary `inherit`-only; postBuildScripts Run Script copies + bottom-up codesigns. Codex Gate 2 caught 10 bugs (2 P0, 6 P1, 2 P2), all resolved before commit. 74 XCTests + 30 pytest = 104 total green. EPIC-05 (Transcript Formatting & Alignment) is now the Active EPIC. |

...
| v0.8 EPIC-09 Build      | 2026-06-04 | Claude Agent | On-device meeting summary (FEA-007) shipped behind a new `SummarizationService` (API-401). `ExtractiveSummarizer` is a deterministic, dependency-free fallback (overview + key points + action-item cue detection) that runs on every macOS 15+ build; `FoundationModelsSummarizer` (Apple on-device LLM) is isolated behind `#if canImport(FoundationModels)` + `@available(macOS 26)` so the macOS-15 SDK build is unaffected (TECH-008); `DefaultSummarizationService` resolves the engine at runtime, preferring the LLM and falling back to extractive on any unavailability/failure (BR-104 — on-device only, no network). Wired a **non-fatal** summarize step into `DefaultPipelineOrchestrator` between format and save (gated by the new `summarize_on_complete` setting, default on); the summary is embedded into the transcript markdown so it reaches both SQLite and the Obsidian export with no DB migration. 9 new XCTests (TEST-601 extractive oracle + TEST-602 orchestrator embed/disable/non-fatal-failure + renderer + service selection). Build verification CI-driven (`macos-15`; FoundationModels path compiles out there as expected). Mac validation (TEST-504 / KPI-001/002 / RISK-008) carried into EPIC-09 Phase D. | EPIC-09, API-401, BR-104, TECH-008, FEA-007, ARC-001, TEST-601→602 |
...
| FEA-007 | On-Device Meeting Summary | P1 (Should) | After transcription, generate an overview + key points + action items on-device and embed them into the transcript markdown (Obsidian + copy). Apple Foundation Models when available (macOS 26+), deterministic extractive fallback otherwise | BR-104, API-401, TECH-008, EPIC-09 |

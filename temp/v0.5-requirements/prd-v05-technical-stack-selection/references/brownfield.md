...
|----------|----------|---------|
| **Reuse** | Existing asset covers the new product's need as-is | "Keycloak handles auth for enRoll. INTERPRET needs the same auth patterns." |
| **Extend** | Existing asset works but needs additions | "React exists but needs new domain-specific components for policy review." |
| **New** | No existing asset for this capability | "No document parsing service exists in the family." |
| **Replace** | Existing asset is wrong for the new product | "Current batch queue doesn't support the real-time pipeline INTERPRET needs." |
...
|----------------|---------------|---------------|----------|-------|
| Frontend | React + Carbon | enRoll | Extend | Need new policy review components |
| Auth | Keycloak on IKS | enRoll | Reuse | Separate realm for INTERPRET |
| Database | PostgreSQL on IBM Cloud | enRoll | Reuse | May need additional schemas |
| Document parsing | None | — | New | Greenfield evaluation needed |
| AI/ML | None | — | New | watsonx.ai evaluation needed |
| Task queue | None | — | New | Pipeline orchestration needed |
